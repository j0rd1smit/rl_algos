...
def main() -> None:
    env_name, reward_scaling_factor, policy = "Taxi-v2", 1.0,  core.categorical_policy
...
def build_pi_model(config: PPOConfig) -> tf.keras.Model:
    ...
    outputs = inputs
    outputs = tf.keras.layers.Embedding(500, 10, input_length=1)(outputs)
    outputs = tf.keras.layers.Reshape((10,))(outputs)
